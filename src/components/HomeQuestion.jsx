    ...
    <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid lg:grid-cols-2 gap-8 items-start">
                {/* Left Image */}
                <img src={faqImage} alt="FAQ" className="w-full h-[420px] object-cover rounded-[30px]"/>
                {/* FAQ */}
                <div className="space-y-4">
                    {faqs.map((faq, index) => (
                        <div key={index} className={`rounded-[22px] transition-all duration-300 ${
                            active === index
                            ? "bg-[#dfe9ff] p-6"
                            : "border-b border-gray-200 pb-4"
                              }`}>
                            <button onClick={() => setActive(active === index ? -1 : index)} className="w-full flex justify-between items-center text-left">
